...
要下载图片或附件，首先需要采集到文件地址。 采集器在自动识别列表时，会自动检测列表中的图片。对于这种，我们直接对图片设置**文件下载**即可。 对于需要下载新的指定位置的文件，我们按照以下方法操作：

1. 点击**添加字段**。
2. 鼠标点击网页中的图片，程序自动获取图片地址。（已有字段，选择**选择**，然后点击图片）
...
## 如何下载多张图片
下载多张图片，我们需要修改图片XPath，来匹配多张图片。 具体XPath如何编写 [参考](/guide/xpath)


## 如何下载文章HTML内的多张图片
采集新闻、文章内容时，如果内容中包含图片，我们需要采集文章的HTML形式，设置[取值属性](/guide/attribute)为`InnerHTML`或者`OuterHTML`。 然后直接设置下载即可，程序会自动检测内容中IMG标签，并且下载图片。
...
2. 使用[动作](/guide/action)中的滚动命令，模拟浏览器的滚动，使得图片自动加载显示。 然后在设置采集。
